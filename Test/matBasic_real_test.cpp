...
//	determinantTest();
//	realMatrixExamples();
	i_real_matrix xyG, dxyRB, mpxyG, leftDiv;
	TestTimer timer;
	timer.tic();
	if (0 <= read_gnuplotdata(xyG, dxyRB)) {
		timer.toc("read_gnuplotdata()");
		std::cout << "rank(xyG) = " << rank(xyG) << "\n";
//		showMatrix(xyG, "mat xyG");
		std::cout << "rank(dxyRB) = " << rank(dxyRB) << "\n";
//		showMatrix(dxyRB, "mat dxyRB");
		timer.tic();
		std::cout << "rank(mpxyG) = " << rank(mpxyG = pinv2(xyG)) <<
			" Moore-Penrose pseudoinverse of xyG\n";
		timer.toc("pinv2(xyG)");
		timer.tic();
		leftDiv = matMul(mpxyG, dxyRB);
		timer.toc("matMul(mpxyG, dxyRB)");
		showMatrix(leftDiv, "matMul(mpinv(xyG), dxyRB)");
	}
	std::cout << "\n done.\n\n";
	return 0;
}
